...
class SimpleTokenizer(object):
    def __init__(
            self,
            bpe_path: str = default_bpe(), # 사용할 BPE 병합 테이블 파일 경로
            additional_special_tokens: Optional[List[str]] = None, # 추가로 사용할 특수 토큰 목록
            context_length: Optional[int] = DEFAULT_CONTEXT_LENGTH, # 문장 최대 토큰 길이
            clean: str = 'lower', # 텍스트 전처리 방식
            reduction_mask: str = '' # 길이 초과 시 어떻게 줄일지
    ):
        self.byte_encoder = bytes_to_unicode() # 바이트 -> 유니코드
        self.byte_decoder = {v: k for k, v in self.byte_encoder.items()} # 유니코드 -> 바이트
        merges = gzip.open(bpe_path).read().decode("utf-8").split('\n')
        merges = merges[1:49152-256-2+1] # 첫 번째 줄 제외하고 가져오기
        merges = [tuple(merge.split()) for merge in merges]
        vocab = list(bytes_to_unicode().values())
        vocab = vocab + [v+'</w>' for v in vocab] # 단어 끝을 나타내는 </w> 추가
        for merge in merges:
        ...
        self.encoder = dict(zip(vocab, range(len(vocab)))) # 문자 -> ID 변환
        self.decoder = {v: k for k, v in self.encoder.items()} # ID -> 문자 변환
        self.bpe_ranks = dict(zip(merges, range(len(merges)))) # BPE 병합 우선순위
        self.cache = {t:t for t in special_tokens} # 특수 토큰 캐싱
        ...
        self.sot_token_id = self.all_special_ids[0] # 시작 토큰 ID
        self.eot_token_id = self.all_special_ids[1] # 종료 토큰 ID
        self.context_length = context_length
        self.clean_fn = get_clean_fn(clean)
        self.reduction_fn = get_reduction_mask_fn(reduction_mask) if reduction_mask else None

    def bpe(self, token): # bpe 토큰화
        if token in self.cache:
            return self.cache[token]
        word = tuple(token[:-1]) + ( token[-1] + '</w>',) # 단어의 마지막 글자에 </w> 추가
        pairs = get_pairs(word) # 단어에서 BPE 병합할 문자 쌍을 추출
    ...
    def encode(self, text): # 텍스트 -> 토큰 ID 변환
        bpe_tokens = []
    ...
    def decode(self, tokens): # 토큰 ID -> 텍스트 변환
        text = ''.join([self.decoder[token] for token in tokens])
        text = bytearray([self.byte_decoder[c] for c in text]).decode('utf-8', errors="replace").replace('</w>', ' ')
        return text

    def __call__(self, texts: Union[str, List[str]], context_length: Optional[int] = None) -> torch.LongTensor: # CLIP 모델이 사용할 최종 텐서를 생성하는 최종 토크나이징 함수
